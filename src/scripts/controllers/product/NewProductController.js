...
export default class NewProductController extends BaseController {
  constructor($scope, $timeout, $stateParams, $state, DbService, ChartService) {
    super(
      ...
      $stateParams,
      $state,
      DbService.getCollection("products"),
      "productList"
    );
    ...
    const numberIsValid = number =>
      typeof number !== "undefined" && number !== null && !isNaN(number);

    $scope.afterChanges = (newVal, oldVal) => {
      if (
        numberIsValid(newVal.cost) &&
        numberIsValid(newVal.price) &&
        (oldVal.cost !== newVal.cost || oldVal.sellMargin !== newVal.sellMargin)
      ) {
        $scope.data.price = newVal.cost + newVal.cost * newVal.sellMargin / 100;
        return true;
      } else if (
        numberIsValid(newVal.cost) &&
        numberIsValid(newVal.sellMargin) &&
        oldVal.price !== newVal.price
      ) {
        $scope.data.sellMargin =
          (newVal.price - newVal.cost) / newVal.cost * 100;
        return true;
      }
    ...
    const getSellingTotalPerPeriod = (start, end, oDeferred) => {
      ...
      sellingsCollection
        .find({
          "items.productId": $scope.data._id,
